...
select dbo.[fnGetEmpFullName](A.first_name, A.last_name) as EmpName, A.manager_id
from employees A

select dbo.[fnGetEmpFullName](B.first_name, B.last_name) as EmpName, B.employee_id  FROM employees B 


select dbo.[fnGetEmpFullName](A.first_name, A.last_name) as EmpName,  dbo.[fnGetEmpFullName](B.first_name, B.last_name)
from employees A
...
select * from 
(select top 10 ROW_NUMBER() OVER(ORDER BY (SELECT 1)) as Rowno, * from employees order by salary desc) a
where Rowno = 2

select * from 
(select top 10 ROW_NUMBER() OVER(ORDER BY (SELECT 1)) as Rowno, salary from employees group by salary order by salary desc) a
where Rowno = 3

select job_id from employees group by job_id order by job_id desc

select * from employees order by first_name desc

select salary,count(1) as cnt from employees group by salary having count(1) > 1
